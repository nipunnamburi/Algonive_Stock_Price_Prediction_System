...
from utils.data_loader import load_historical_data, load_company_info
...
selection_mode = st.sidebar.radio("Select input mode:", ["Select Predefined", "Search Custom Ticker"])

if selection_mode == "Select Predefined":
...
start_date = datetime(end_date.year - 5, end_date.month, end_date.day) # 5 years of history for analysis
...
if ticker_input:
    ...
    if hist_data.empty:
        ...
        with col2:
            st.metric(label="52-Week High", value=f"${highest_52w:.2f}")
        with col3:
            st.metric(label="52-Week Low", value=f"${lowest_52w:.2f}")
        with col4:
            st.metric(label="Daily Volume", value=format_volume(last_row['Volume']))
        with col5:
            st.metric(label="Market Cap", value=format_large_number(company_info['market_cap']))
        ...
        with chart_col:
            st.plotly_chart(plot_historical_prices(processed_data, ticker_input), use_container_width=True)
        ...
        with table_col:
            ...
            st.markdown(f"""
            - **Current SMA 20:** ${last_row['SMA_20']:.2f}
            - **Current SMA 50:** ${last_row['SMA_50']:.2f}
            - **Current EMA 20:** ${last_row['EMA_20']:.2f}
        ...
        if predict_clicked:
            ...
            with fore_chart_col:
                ...
                st.plotly_chart(plot_forecast(processed_data, forecast, ticker_input), use_container_width=True)
            ...
            with fore_table_col:
                ...
                forecast_df = pd.DataFrame({
                    "Day": days,
                    "Date": pd.date_range(start=datetime.now() + timedelta(days=1), periods=forecast_len, freq='B').strftime('%Y-%m-%d'),
                    "Prediction ($)": [round(p, 2) for p in forecast]
                })
    ...
    st.info("👈 Choose a predefined stock or search a custom ticker in the sidebar to get started.")
